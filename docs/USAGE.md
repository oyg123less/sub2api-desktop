...
<a id="cloud"></a>
<details>
<summary><strong>09 · 云账户、连接码快速共享与好友管理</strong></summary>
...
- **概览**：最上方直接提供“我的共享”和“连接他人的共享”两个主要任务。
- **我的共享**：选择账号并生成 9 位连接码和 6 位临时密码。
- **收到的共享**：接收 Base URL 和独立 API Key。
- **好友**：作为常用联系人保留，不再是快速共享的前置条件，也不提供聊天。
...
1. 在云账户“概览”找到“我的共享”。
2. 点击“选择共享账号”，只勾选愿意共享的账号并保存账号池。
3. 点击“开始共享”，选择允许连接人数和密码有效时间。
4. 点击“复制连接信息”，把整段连接码和临时密码发给接收者。

接收者使用：

1. 登录自己的 Amber 云账户。
2. 在“连接他人的共享”中点击“粘贴连接信息”，也可以手动输入连接码和临时密码。
3. 点击“连接并使用”。成功后共享会自动加入 Amber 本地调度，无需手工复制 Base URL 或 API Key。

同一组连接码和临时密码可以按共享者设置供 1 到 20 人领取，但每个人都会获得**独立 Guest Key**。共享者可以在“已连接用户”中单独暂停或移除某个人，不影响其他接收者。刷新临时密码只阻止新的领取，不会使已连接用户立刻掉线；“暂停共享”会同时阻止新连接和已有调用，恢复后可继续使用。

> **连接码和临时密码不是模型 API Key。** 它们只在短时间内用于领取权限。Worker 只保存 Guest Key 哈希和接收者自行加密的信封，不保存 Guest Key 明文。

### 本机回源、在线要求和代理

- OAuth 账号必须通过共享者的 Amber Agent 回源。共享者的设备、Amber 和“允许本机回源”需要保持在线。
- API Key 账号可以在账号池中选择“本机回源”或“Worker 直连”。
- 接收者加入本地调度后，可以停用该共享；其请求不会自动继承共享者以外的代理配置。
- 共享者刷新密码、暂停共享或移除接收者后，状态会在下一次云工作区刷新时同步。

### 高级：好友共享组

旧的好友共享组仍保留在“我的共享”和“好友”标签中，适合需要命名多个共享池、预先指定好友和详细规则的用户：
...
云端启用最低版本要求后，旧版 Amber 打开云账户会收到明确的更新提示。点击“下载并安装最新版本”前往 GitHub Releases。直接覆盖安装会保留本地数据库、账号、代理和设置；不需要先卸载旧版本。
