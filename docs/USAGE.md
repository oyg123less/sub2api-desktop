...
> 从账号导入、本地 API 接入，到工作区隔离、设备定向回流和多账号共享的完整说明。

本手册中的账号、域名、连接码和密钥均为演示数据，不包含真实用户信息。点击下面任一章节标题即可展开图文步骤。
...
<a id="cloud"></a>
<details>
<summary><strong>09 · 云账户、连接码共享与设备定向回流</strong></summary>
...
- **共享中心**：左侧发起共享，右侧输入连接码和临时密码；下方查看已接收授权。
- **设备**：开启本机回源，并固定一台主设备和最多两台备用设备。
- **账户与安全**：查看同步、冲突、身份密钥和工作区状态。

v0.4.4 不再显示好友、好友申请或旧共享组页面。升级不会删除旧云端记录，只是把它们移出日常操作界面。
...
首次发起共享时必须在“设备”中选择承载回流的主设备。需要容灾时再显式添加备用设备，不会在同一云账号的所有在线电脑之间随机选路。
...
- OAuth 账号必须通过共享者选定的 Amber Agent 回源。主设备、Amber 和“允许本机回源”需要保持在线。
- 主设备在请求尚未访问上游时不可用，Worker 才会按顺序尝试显式备用设备；上游请求一旦开始，绝不向另一设备重放。
...
v0.4.4 为每个云用户建立独立本地工作区。第一次把未绑定的旧数据绑定给某个云账户后：

- 退出并登录另一个云账户会切换到它自己的账号、代理、设置、同步队列和 Codex 目标。
- 新用户没有工作区时会得到全新的空工作区，不会继承上一个用户的数据。
- 已绑定工作区不能被另一个云用户接管。
- 发现旧目录里存在无法确认归属的多用户历史数据时，Amber 进入只读恢复模式，禁止猜测归属和自动同步。

请在“管理工作区”中确认当前邮箱和工作区 ID，再进行导入、同步或共享。

### 账号网络模式

- **强制直连**：忽略系统代理环境，适合不需要代理的 Base URL。
- **跟随系统**：使用操作系统代理设置。
- **指定代理**：只使用该账号选择的 Amber 代理。

删除指定代理后，账号会安全回落为强制直连。共享池中某个账号网络失败时只隔离该账号，不应拖垮整个共享池。
