...
- **客户端兼容模式**：可选 Codex 指令与实验性 TLS profile，用于协议兼容，不承诺规避风控
...
项目按 LGPL-3.0-or-later 提供，第三方组件许可见 `THIRD_PARTY_NOTICES.md`。使用非官方转发存在账号被 OpenAI 限制的风险；客户端兼容模式无法保证账号安全，请自行评估并遵守相关服务条款。
