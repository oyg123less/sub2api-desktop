...
const localStatus = { version: "0.4.2", server_running: true, port: 8080, host: "127.0.0.1", endpoint: "http://127.0.0.1:8080/v1", lan_addresses: [], local_api_key: "", account_count: 2, schema_version: 14 };
...
test("shows an installer-oriented upgrade screen for rejected old clients", async ({ page }) => {
  ...
  await page.route(`${controlOrigin}/control/**`, async (route) => {
    ...
    if (path === "/control/cloud/workspace") return fulfill(route, {
      error: { code: "client_upgrade_required", message: "Update Amber", retryable: false, details: { minimum_version: "0.4.2", latest_version: "0.4.2", update_url: "https://github.com/oyg123less/sub2api-desktop/releases/latest" } },
    }, 426);
    return fulfill(route, {});
  });
  await page.goto("/#/cloud");
